...
public interface UnidadeFederativaRepository extends JpaRepository<UnidadeFederativa, String> {
    ...
    @Query("SELECT CASE WHEN COUNT(u) > 0 THEN true ELSE false END FROM UnidadeFederativa u WHERE u.siglaUF = :siglaUF")
    boolean existsBySiglaUF(@Param("siglaUF") String siglaUF);

    @Query("SELECT u FROM UnidadeFederativa u WHERE u.siglaUF = :siglaUF")
