...
public interface LogradouroRepository extends JpaRepository<Logradouro, Long> {
    ...
    @Query("SELECT CASE WHEN COUNT(l) > 0 THEN true ELSE false END FROM Logradouro l WHERE l.nomeLogradouro = :nomeLogradouro")
    boolean existsByNomeLogradouro(@Param("nomeLogradouro") String nomeLogradouro);

    @Query("SELECT l FROM Logradouro l WHERE l.nomeLogradouro = :nomeLogradouro")
